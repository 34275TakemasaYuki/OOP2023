...
namespace Test02 {
    class Program {
        ...
        private static void Exercise02(List<int> numbers) {
            var numsObj = numbers.Where(n => n % 2 == 0).Max();
            Console.WriteLine(numsObj);

        }
        //問題３　昇順に並べて表示（遅延実行とする）
        //　　　　出力結果【12 14 17 20 31 35 40 48 53 76 87 91 94】
        private static void Exercise03(List<int> numbers) {
            var numsObj = numbers.OrderBy(n => n);
            foreach (var nums in numsObj)
            {
                Console.Write(nums + " ");
            }
        ...
        private static void Exercise04(List<int> numbers) {
            var numsObj = numbers.Where(n => n >= 10 && n <= 50);
            foreach (var nums in numsObj)
            {
                Console.Write(nums + " ");
            }
